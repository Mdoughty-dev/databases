...
const seedArticles = (articleData) => {
  const articlePromises = articleData.map(article => {
    return db.query(`
      ...
      RETURNING *;
    `, [article.title, article.topic, article.author, article.votes, article.body, article.created_at, article.article_img_url])
      .catch(err => {
    ...
    .then(result => {
      if (result.rows.length > 0) {
        ...
        return db.query(`
          ...
          VALUES ($1, $2, $3, $4, $5)
          RETURNING *;
        `, [article_id, comment.body, comment.votes, comment.author, comment.created_at])
          .catch(err => {
            console.error('Error inserting comment:', err);
          });
      } else {
        console.error(`No article found with title: ${comment.article_title}`);
      }
    })
    .catch(err => {
      console.error('Error fetching article_id for title:', comment.article_title, err);
    });
  });
